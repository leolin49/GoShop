...
func ProductClientStart() error {
	...
	if err != nil || addr == "" {
		glog.Errorln("[StockServer] consul service recover failed.")
		return err
	}
	product_conn, err = grpc.NewClient(addr, grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		glog.Errorln("[StockServer] new product rpc client error: ", err.Error())
		return err
	}
	product_client = productpb.NewProductServiceClient(product_conn)
	glog.Infoln("[StockServer] connect [product-service] server successful on: ", addr)
	return nil
}
